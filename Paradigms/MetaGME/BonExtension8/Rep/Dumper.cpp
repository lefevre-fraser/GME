...
{
	std::string res;
	ASSERT(0); // it this good?
	std::set<CComPtr<IMgaModel> >::iterator it = m_setOfParadigmSheets2.begin();
	for( ; it != m_setOfParadigmSheets2.end(); ++it)
	{
		CComPtr<IMgaRegNodes> rns;
		COMTHROW( (*it)->get_Registry( VARIANT_TRUE, &rns));
...
{
	CComPtr<IMgaRegNode> rn;
	COMTHROW( m_BON_Project_Root_Folder2->get_RegistryNode( Util::Copy( std::string( "BonExtender/SavedConfigurations")), &rn));
	if ( rn)
	{
		CComPtr<IMgaRegNodes> rns;
		COMTHROW( rn->get_SubNodes( VARIANT_TRUE, &rns));
		...
		for( long i = 1; i <= c; ++i)
		{
			...
			std::string one_config = Util::Copy( nm);

			CComBSTR val;
			COMTHROW( item->get_Value( &val));
...
void Dumper::selConf( std::vector<FCO*>& s)
{
	...
	for( std::vector<FolderRep*>::const_iterator it_b = m_folderList.begin(); it_b != m_folderList.end(); ++it_b)
	{
		...
		std::string regVal = Util::gvbp( (*it_b)->getMyRegistry(), "Configs/Default");
	...
	for( std::vector< FCO *>::iterator it = s.begin(); it != s.end(); ++it)
	{
		...
		std::string regVal = Util::gvbp( (*it)->getMyRegistry(), "Configs/Default");
		std::string rspVal = Util::gvbp( (*it)->getMyRegistry(), "Configs/Default/Resp");
		FCO* resp_ptr = (*it)->findRspPtr( rspVal);
		if( !resp_ptr) // valid info in Responsible registry?
		{
			rspVal = "";
			Util::svbp( (*it)->getMyRegistry(), "Configs/Default/Resp", rspVal);
		}
	...
	if ( !global_vars.silent_mode && sc.DoModal() == IDOK)
	{
		...
		if ( sc.m_currSelConfigStr != "Default")
		{
			...
			CComPtr<IMgaRegNode> rn;
			COMTHROW( m_BON_Project_Root_Folder2->get_RegistryNode( Util::Copy( path), &rn));
			ASSERT( rn);
			COMTHROW( rn->put_Value( Util::Copy( std::string( "1"))));
		}
	}	
}
...
/*static*/ int Dumper::selectOutputFiles( const std::string& proj_name_to_be_checked, CComPtr<IMgaRegNode> optRn)
{
	...
	std::string _visitor = Util::gvbp( optRn, Dumper::m_strGenVisitor);
	std::string _vissign = Util::gvbp( optRn, Dumper::m_strVisitorSignature);
	std::string _initmtd = Util::gvbp( optRn, Dumper::m_strGenInitMethod);
	std::string _finimtd = Util::gvbp( optRn, Dumper::m_strGenFiniMethod);
	std::string _acceptT = Util::gvbp( optRn, Dumper::m_strGenAcceptWithTraversal);
	std::string _acceptS = Util::gvbp( optRn, Dumper::m_strGenAcceptSpecialized);
	std::string _acSRetV = Util::gvbp( optRn, Dumper::m_strSpecAcceptRetVal);
	std::string _vcVers6 = Util::gvbp( optRn, Dumper::m_strVCVersion6);
	std::string _templGt = Util::gvbp( optRn, Dumper::m_strTemplateGetter);
	std::string _outSepF = Util::gvbp( optRn, Dumper::m_strOutputInSeparateFile);
	std::string _tgtNmsp = Util::gvbp( optRn, Dumper::m_strTgtNamespace);
	...
	if( !global_vars.silent_mode) // show dialog only in verbose mode
	{
		...
		Util::svbp( optRn, Dumper::m_strGenVisitor            , m_bGenVisitor        ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strVisitorSignature      , m_iVisitSign == 1    ? "1" : "0");
		Util::svbp( optRn, Dumper::m_strGenInitMethod         , m_bGenInit           ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strGenFiniMethod         , m_bGenFinalize       ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strGenAcceptWithTraversal, m_bGenAcceptTrave    ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strGenAcceptSpecialized  , m_bGenAcceptSpeci    ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strSpecAcceptRetVal      , m_iSpecAcceptRetVal == 1 ? "1" : "0");
		Util::svbp( optRn, Dumper::m_strVCVersion6            , m_bGenTemplVersion6  ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strTemplateGetter        , m_bGenTemplates && m_bGenRegular ? Yes_Str : m_bGenTemplates ? Only_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strOutputInSeparateFile  , global_vars.m_outputMethod == Globals::OUTPUTMETHOD_ENUM::PERCLASS ? Yes_Str : No_Str);
		Util::svbp( optRn, Dumper::m_strTgtNamespace          , global_vars.m_namespace_name);
