			...
			for ( ; kt != part_series.end(); ++kt)
			{
				...
				if ( !primary_aspect_needed)
				{
						...
						try {
							...
							Util::svbp( part.getFCOPtr()->getMyRegistry(), primary_reg_path, regVal);
						}
						catch(...) {
				...
				else if ( is_model)
				{
					...
					if ( sub_model_no_of_aspects <= 1) // no more than one aspect
					{
						...
						try {
							//part.getFCOPtr()->getPtr()->getRegistry()->setValueByPath( regPath, regVal);
							Util::svbp( part.getFCOPtr()->getMyRegistry(), kindasp_reg_path, regVal);
						}
						catch(...) {
							global_vars.err << MSG_ERROR << "Internal error: Exception during registry related write operation.\n";
						}
				...
				if (!skip)
				{
					...
					if ( primary_aspect_needed)
					{
						...
						std::string old_regVal1 = Util::getReg( part.getFCOPtr()->getPtr(), primary_reg_path);
						std::string old_regVal2 = Util::gvbp( part.getFCOPtr()->getMyRegistryOld(), primary_reg_path);
						regVal = Util::gvbp( part.getFCOPtr()->getMyRegistry(), primary_reg_path);
					...
					if ( kind_aspect_needed && (is_model || is_ref_to_model)) // is there need for kind_asp_name?
					{
						...
						std::string old_regVal1 = Util::getReg( part.getFCOPtr()->getPtr(), kindasp_reg_path);
						std::string old_regVal2 = Util::gvbp( part.getFCOPtr()->getMyRegistryOld(), kindasp_reg_path);
						regVal = Util::gvbp( part.getFCOPtr()->getMyRegistry(), kindasp_reg_path);
						if ( regVal.empty() && ( !old_regVal1.empty() || !old_regVal2.empty()))
						{
							regVal = old_regVal2.empty()?old_regVal1:old_regVal2;
		...
		while ( it != asp_map.end())
		{
			...
			if ( !selected_primary_value.IsEmpty() && (selected_primary_value.Compare("no") == 0 || selected_primary_value.Compare("yes") == 0))
			{
				...
				try {
					...
					Util::svbp( act_part.getFCOPtr()->getMyRegistry(), regPath, regVal);
				} catch( ...) {
					global_vars.err << MSG_ERROR << "Internal error: Exception during registry related write operation.\n";
				}
			}
...
bool Dumper::aspectOrderSel()
{
	...
	COMTHROW( Sheet::m_BON_Project_Root_Folder->get_RegistryNode( Util::Copy( AspectRep::m_aspectRegistryRoot + aspect_order_registry_token), &rn));
	CComBSTR value;
	if( rn) COMTHROW( rn->get_Value( &value));
	if ( rn && Util::Copy(value) == "valid") // fetch priority values from the Registry
	{
		...
		CComPtr<IMgaRegNodes> ch;
		COMTHROW( rn->get_SubNodes( VARIANT_TRUE, &ch));

		long d = 0;
		if( ch) COMTHROW( ch->get_Count( &d));

		for( long i = 1; i <= d; ++i)
		{
			CComPtr<IMgaRegNode> r_item;
			COMTHROW( ch->get_Item( i, &r_item));
			
			CComBSTR r_name;
			COMTHROW( r_item->get_Name( &r_name));

			CComBSTR r_value;
			COMTHROW( r_item->get_Value( &r_value));
			std::string value = Util::Copy( r_value);
			int k = 0;
			int sc = sscanf( value.c_str(), "%d", &k);
			ASSERT( sc == 1);
			asp_order.insert( AspNamePriorityComp::Type( Util::Copy( r_name), k));
		}
	...
	if ( dlg_res == IDOK)
	{
		...
		COMTHROW( Sheet::m_BON_Project_Root_Folder->put_RegistryValue( Util::Copy( path), L"valid"));
		...
		for( ; i < g.m_resultAspects.size(); ++i)
		{
			...
			COMTHROW( Sheet::m_BON_Project_Root_Folder->put_RegistryValue( Util::Copy( path + subtoken), Util::Copy( std::string( idx_str))));//similar aspects will use the same priority by overwriting the values in the same regnode
		}
...
/*static*/ int Dumper::selectOutputFiles( IMgaProject* proj, const std::string& p_proj_name, const std::string& proj_path)
{
...
/*static*/ int Dumper::selectOptions( IMgaProject* proj) 
{
