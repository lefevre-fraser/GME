...
class regexp : public CRegProgramAccessor
{
	...
	int m_programSize;
	LPTSTR startp[CRegExp::NSUBEXP];
	LPTSTR endp[CRegExp::NSUBEXP];
	...
	bool status;
	int count;			// used by CRegExp to manage the reference counting of regexps
	int numSubs;
public:
	...
	regmust(0)
{
	...
	for ( int i = CRegExp::NSUBEXP; i > 0; i--)
	{
...
CRegExp::CRegExp( LPCTSTR exp, BOOL iCase )
	: rc( new regexp( exp, iCase ) ),
	string( 0 )
{
}

CRegExp::CRegExp( const CRegExp &r )
	: rc( r.rc ),
...
bool CRegExp::Match( const TCHAR * s )
{
	ClearErrorString();
	string = s;
	bool ret = false;
	if ( rc )
	{
		...
		int i = 0;
		if ( ret )
			for ( i = 0; i < CRegExp::NSUBEXP && rc->startp[i] ; i++ )
				;
		rc->numSubs = i - 1;
	}
	else
...
CString CRegExp::GetReplaceString( LPCTSTR source ) const
{
	ClearErrorString();
	if ( rc )
...
int CRegExp::safeIndex( unsigned int i ) const
{
	return i < CRegExp::NSUBEXP ? i : CRegExp::NSUBEXP;
}

const CString CRegExp::operator[]( unsigned int i ) const
{
	ClearErrorString();
	ASSERT( rc );
	if ( rc )
...
void CRegExp::ClearErrorString() const
{
	if ( rc )
		rc->ClearErrorString();
	m_szError.Empty();
