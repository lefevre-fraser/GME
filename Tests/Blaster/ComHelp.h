...
extern void SendMail(CString msg);
extern CString ltoCStr(long n);
extern void LogWrite(CString text);

extern CString lastLogEntry;

extern ofstream toPattFile;
extern ifstream fromPattFile;
extern ofstream out;
...
//	VERIFY(SUCCEEDED(hr));
	if(!SUCCEEDED(hr))
	{
		VERIFY(("COMVERIFY has failed in the Blaster.  Press Ignore.",false));
		SendMail("Blaster failed with error " + ltoCStr(hr) + ".\nLast
			log entry:\n\n" + lastLogEntry);
		if(toPattFile)
		{
			toPattFile.close();
		...
		if(out)
		{
			LogWrite("Blaster failed with error " + ltoCStr(hr));
			out.close();
		}
		throw("COMVERIFY has failed.");
	}
}
#endif
...
{																				\
	CString errString;															\
	sprintf(errString.GetBuffer(255), "Error %08lX: %s\n", code, string);		\
	errString.ReleaseBuffer();													\
	SendMail("Blaster failed at line " + ltoCStr(__LINE__) +": " + errString + "\nLast log entry:\n" + lastLogEntry); \
	if(toPattFile)																\
	{																			\
		toPattFile.close();														\
	}																			\
	...
	if(out)																		\
	{																			\
		LogWrite("Blaster failed at line " + ltoCStr(__LINE__) +" : " + errString);								\
		out.close();															\
	}																			\
	throw("COMVERIFY has failed.");												\
}
...
{																				\
	...
	catch(const char *cvhf)														\
	{																			\
		if(strcmp("COMVERIFY has failed.",cvhf) == 0)							\
			throw;																\
	}																			\
	catch(...)																	\
	{																			\
		PRINTANDTHROW(0x87658000,#xx);											\
	}																			\
}

/*
	catch(hresult_exception &he)												\
	{																			\
		CString excStr = " (code from caught hresult_exception)";				\
		excStr.Insert(0,#xx);													\
		PRINTANDTHROW(he.hr,excStr);											\
	} 																			\
